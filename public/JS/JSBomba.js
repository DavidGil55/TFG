...
botonCambio.addEventListener("click", () => {
    ...
    mensajesChat.style.display = "flex";
...
inputPalabra.addEventListener("keydown", evento => {
    if (evento.key === "Enter") {
        ...
        if (palabraLimpia.length > 0) {
            socket.emit("enviar-bomba", palabraLimpia);
            inputPalabra.value = "";
        }
    }
});
...
socket.on("estado-juego", estado => {
    ...
    jugadores.forEach((jugador, indice) => {
        ...
        // Posicionamiento de las tarjetas de los jugadores
    ...
    if (estado.enJuego && !estado.preparando && nJugadores > 0) {
        ...
        inputPalabra.disabled = !esMiTurno; // Solo se habilita el input si es tu turno y estás vivo.
        inputPalabra.placeholder = esMiTurno ? "TU TURNO! Escribe algo..." : "Espera tu turno...";
        // Se le pone el focus al input para la bomba 
        if (esMiTurno) inputPalabra.focus();
    } else { // Si no es tu turno...
        inputPalabra.disabled = true;
        inputPalabra.placeholder = estado.preparando ? "Prepárate..." : "Esperando jugadores...";
    }
...
socket.on("estado-sala", objetoSala => {
    ...
    // Que stringifique el objeto "objetoSala", que no reemplace nada, y que tenga 4 espacios de identación. 
